-- Load generated CSV data into PostGIS database
...
\echo ''
\echo '============================================'
\echo 'Loading data into PostGIS database...'
\echo '============================================'

-- Load neighborhoods
\echo ''
\echo '1. Loading neighborhoods...'
\copy neighborhoods(neighborhood_id, name, boundary, population) FROM '/data/neighborhoods.csv' WITH (FORMAT csv, HEADER true);
SELECT COUNT(*) AS neighborhoods_loaded FROM neighborhoods;

-- Load street lights
\echo ''
\echo '2. Loading street lights...'
\copy street_lights(light_id, location, status, wattage, installation_date, last_maintenance, neighborhood_id) FROM '/data/street_lights.csv' WITH (FORMAT csv, HEADER true);
...
\copy maintenance_requests(request_id, light_id, reported_at, resolved_at, issue_type) FROM '/data/maintenance_requests.csv' WITH (FORMAT csv, HEADER true, NULL '');
SELECT COUNT(*) AS requests_loaded FROM maintenance_requests;

-- Load suppliers
\echo ''
\echo '4. Loading suppliers...'
\copy suppliers(supplier_id, name, location, contact_phone, service_radius_km, avg_response_hours, specialization) FROM '/data/suppliers.csv' WITH (FORMAT csv, HEADER true);
SELECT COUNT(*) AS suppliers_loaded FROM suppliers;

-- Load weather enrichment
\echo ''
\echo '5. Loading weather enrichment...'
\copy weather_enrichment(light_id, season, avg_temperature_c, rainfall_mm, failure_risk_score, predicted_failure_date) FROM '/data/weather_enrichment.csv' WITH (FORMAT csv, HEADER true, NULL '');
...
\copy demographics_enrichment(neighborhood_id, population_density, urban_classification) FROM '/data/demographics_enrichment.csv' WITH (FORMAT csv, HEADER true);
...
\copy power_grid_enrichment(light_id, grid_zone, avg_load_percent, outage_history_count) FROM '/data/power_grid_enrichment.csv' WITH (FORMAT csv, HEADER true);
...
FROM street_lights l
WHERE NOT EXISTS (
    SELECT 1 FROM neighborhoods n WHERE ST_Within(l.location, n.boundary)
);
...
SELECT status, COUNT(*) AS count, 
       ROUND(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM street_lights), 2) AS percentage
FROM street_lights
GROUP BY status
ORDER BY count DESC;
...
SELECT 
    (SELECT COUNT(DISTINCT light_id) FROM weather_enrichment) AS lights_with_weather,
    (SELECT COUNT(DISTINCT light_id) FROM power_grid_enrichment) AS lights_with_power_grid,
    (SELECT COUNT(DISTINCT neighborhood_id) FROM demographics_enrichment) AS neighborhoods_with_demographics,
    (SELECT COUNT(*) FROM street_lights) AS total_lights,
    (SELECT COUNT(*) FROM neighborhoods) AS total_neighborhoods;
